 ...
 * overrides are located in their respective config/environments/{{WP_ENV}}.php files.
 *
 ...
 */
if ( ! defined( 'BROCOOLY_APP_PATH' ) ) {
	define( 'BROCOOLY_APP_PATH', dirname( __DIR__ ) );
}
 ...
 */
$webroot_dir = BROCOOLY_APP_PATH . '/web';
 ...
 */
$env_files = file_exists( BROCOOLY_APP_PATH . '/.env.local' )
	? [ '.env', '.env.local' ]
	: [ '.env' ];

$dotenv = Dotenv\Dotenv::createUnsafeImmutable( BROCOOLY_APP_PATH, $env_files, false );
if ( file_exists( BROCOOLY_APP_PATH . '/.env' ) ) {
	$dotenv->load();
...
if ( $debug_log && class_exists( 'Carbon\Carbon' ) ) {
	...
	$dir  = BROCOOLY_APP_PATH . '/web/app/themes/brocooly' . DIRECTORY_SEPARATOR . $debug_log;

	if ( ! file_exists( $dir ) ) {
		mkdir( $dir, 0777, true );
	}

	$debug_log = $dir . DIRECTORY_SEPARATOR . 'brocooly-' . $date . '.log';
}
